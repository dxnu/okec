...
auto default_decision_engine::make_decision(const task_element& header) -> result_t
{
    ...
    if (cpu_supply > 0) {
        ...
        if (processing_time < big_float(header.get_header("deadline"))) {
            ns3::Vector position(TO_DOUBLE(device_max["pos_x"]), TO_DOUBLE(device_max["pos_y"]), TO_DOUBLE(device_max["pos_z"]));
    ...
    if (it != this->cache().end()) {
        ...
        ns3::Vector position(TO_DOUBLE(device["pos_x"]), TO_DOUBLE(device["pos_y"]), TO_DOUBLE(device["pos_z"]));
...
auto default_decision_engine::send(task_element& t, client_device* client) -> bool
{
    ...
    if (this->local_test(t, client)) {
        ...
        // 执行任务
        ns3::Simulator::Schedule(ns3::Seconds(time), +[](const ns3::Time& time, double processing_time, 
        ...
        }, ns3::Simulator::Now(), time, client, t, old_cpu_cycles);
    } else { // 发送到远端处理
        // 追加任务发送地址信息
    ...
    cloud_server* cs, ns3::Ptr<ns3::Packet> packet, const ns3::Address& remote_address) -> void
{
    ...
    fmt::print(fg(fmt::color::yellow), "At time {:.2f}s The Cloud server[{:ip}] is processing the task(task_id = {}).\n", 
        ns3::Simulator::Now().GetSeconds(), cs->get_address(), task_id);
    ...
    // 执行任务
    ns3::Simulator::Schedule(ns3::Seconds(time), +[](const ns3::Time& time, double processing_time, cloud_server* self, const std::string& task_id, const ns3::Ipv4Address& desination) {
    ...
    }, ns3::Simulator::Now(), time, cs, task_id, inetRemoteAddress.GetIpv4());
}
    ...
    edge_device* es, ns3::Ptr<ns3::Packet> packet, const ns3::Address& remote_address) -> void
{
    ...
    fmt::print(fg(fmt::color::yellow), "At time {:.2f}s The edge server[{:ip}] is processing the task(task_id = {}).\n", 
        ns3::Simulator::Now().GetSeconds(), es->get_address(), task_id);
    ...
    // 执行任务
    ns3::Simulator::Schedule(ns3::Seconds(time), +[](double processing_time, edge_device* es, const std::string& old_cpu_cycles, const std::string& task_id, const ns3::Ipv4Address& desination) {
        // 释放资源
        auto device_resource = es->get_resource();
...
{
    fmt::print(fg(fmt::color::red), "At time {:.2f}s Client device[{:ip}] receives", ns3::Simulator::Now().GetSeconds(), client->get_address(), client->get_port());
